...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("market_data_cache.log")],
)
...
def fetch_and_cache_from_yahoo(
    db: Session, symbol: str, time_period: str, cache_duration_hours: int
) -> bool:
    """
    ...
    try:
        ...
        df = yf.download(symbol, period=time_period, progress=False, interval="1d")
        ...
        if success:
            logger.info(
                f"Successfully cached Yahoo Finance data for {symbol} ({time_period})"
            )
            return True
        else:
            logger.warning(
                f"Failed to cache Yahoo Finance data for {symbol} ({time_period})"
            )
            return False
...
def fetch_and_cache_from_alpha_vantage(
    ...
    try:
        ...
        if success:
            logger.info(
                f"Successfully cached Alpha Vantage data for {symbol} ({time_period})"
            )
            return True
        else:
            logger.warning(
                f"Failed to cache Alpha Vantage data for {symbol} ({time_period})"
            )
            return False
...
def fetch_and_cache_from_fmp(
    ...
    if not FMP_API_KEY:
        logger.warning(
            "Financial Modeling Prep API key not found in environment variables"
        )
        return False
...
def cache_symbol_data(
    ...
    if fetch_and_cache_from_alpha_vantage(
        db, symbol, time_period, cache_duration_hours
    ):
        return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cache market data from various sources"
    )
    parser.add_argument(
        "--symbols", type=str, help="Comma-separated list of symbols to fetch"
    )
    parser.add_argument(
        "--periods", type=str, help="Comma-separated list of time periods to fetch"
    )
    parser.add_argument(
        "--cache-hours",
        type=int,
        default=24,
        help="How long to cache the data in hours",
    )
    ...
    logger.info(
        f"Starting to cache data for {len(symbols)} symbols and {len(periods)} time periods"
    )
    ...
    try:
        ...
        logger.info(
            f"Completed caching data for {len(symbols)} symbols and {len(periods)} time periods"
        )
