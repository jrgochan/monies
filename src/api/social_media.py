...
def get_twitter_trending_topics(
    search_term, api_key, api_secret, access_token, access_secret
):
    """
...
def post_to_twitter(
    message, api_key, api_secret, access_token, access_secret, media_path=None
):
    """
    Post content to Twitter.
    """
    result = {"success": False, "message": "", "post_id": None}
...
def publish_scheduled_posts():
    ...
    try:
        ...
        for post in posts:
            try:
                ...
                if post.platform == "twitter":
                    ...
                    result = post_to_facebook(token, post.content, post.media_path)

                elif post.platform == "linkedin":
                    result = post_to_linkedin(token, post.content, post.media_path)
        ...
        return {"success": True, "message": f"Published {len(posts)} scheduled posts"}
