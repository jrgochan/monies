        ...
        return pd.DataFrame(
            {
                ...
                "error": [True],
                "error_message": [
                    "No portfolio data available or cannot connect to price source."
                ],
            }
        )
    ...
    return pd.DataFrame(
        {
            ...
            "error": [True],
            "error_message": [
                "Cannot retrieve historical data. Please check your network connection."
            ],
        }
    )
...
def show_dashboard():
    ...
    if not wallets:
        st.info(
            "You don't have any wallets configured yet. Go to the Wallets page to add one."
        )
        return
    ...
    if "error" in prices:
        ...
        st.warning(
            "Cannot retrieve current portfolio data. Please check your network connection and try again."
        )
    ...
    with col1:
        ...
        if "error" in prices:
            st.metric(label="Total Portfolio Value", value="Error", delta=None)
        else:
            st.metric(
    ...
    with col2:
        st.metric(label="Crypto Assets", value=f"{len(portfolio)}", delta=None)

    with col3:
        st.metric(label="Connected Wallets", value=f"{len(wallets)}", delta=None)
    ...
    with tab1:
        ...
        if "error" in portfolio_history.columns and portfolio_history["error"].iloc[0]:
            ...
            st.warning(
                "Cannot display portfolio performance chart. Please check your network connection and try again."
            )
        else:
            # Plot line chart
            fig = px.line(
                portfolio_history, x="date", y="value", title="30-Day Portfolio Value"
            )
            fig.update_layout(
                xaxis_title="Date",
    ...
    with tab2:
        ...
        if "error" in prices:
            st.error(prices["error"])
            st.warning(
                "Cannot display asset allocation. Please check your network connection and try again."
            )
        else:
            ...
            if allocation:
                ...
                if labels and values:
                    ...
                    st.info(
                        "Insufficient data for allocation chart. Check your wallet balances."
                    )
            else:
                st.info(
                    "No asset allocation data available. Try adding cryptocurrency to your wallets."
                )
