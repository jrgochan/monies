...
def show_crypto_trading():
    ...
    if not wallets:
        st.warning(
            "You need to add a wallet before trading. Go to the Wallets page to add one."
        )
        return
    ...
    if not exchange_wallets:
        st.warning(
            "You need to add an exchange wallet to trade. Go to the Wallets page to add one."
        )
        return
    ...
    selected_wallet_name = st.selectbox(
        "Select Exchange Wallet", options=list(wallet_options.keys())
    )
    ...
    with st.form("trading_form"):
        ...
        if order_type == "limit":
            price = st.number_input(
                f"Price ({quote_currency})", min_value=0.0, step=0.01
            )
        ...
        if submitted:
            if amount <= 0:
                ...
                api_key, api_secret = get_api_key(
                    db, user["id"], selected_wallet.exchange
                )
                db.close()
                    ...
                    with st.spinner("Placing order..."):
                        ...
                        if result["success"]:
                            ...
                            if add_transaction(
                                ...
                                st.warning(
                                    "Order placed on exchange, but failed to record in local database."
                                )
                        else:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    dates = pd.date_range(
                        end=datetime.now(), periods=252, freq="B"
                    )  # 1 year of business days
                    close_prices = 100 + np.cumsum(
                        np.random.normal(0, 1, len(dates)) / 100
                    )
            ...
            with col1:
                ...
                with stats_col3:
                    st.metric(
                        "Expense Ratio",
                        f"{info.get('expenseRatio', 0) * 100:.2f}%"
            ...
            with col2:
                ...
                if not hist.empty and "Close" in hist.columns:
                    fig = px.line(hist, y="Close", title=f"{etf_ticker} Price (1 Year)")
            ...
            with st.form("etf_invest_form"):
                amount_usd = st.number_input(
                    "Investment Amount (USD)", min_value=0.0, step=100.0
                )
                ...
                if fund_source == "Existing Crypto":
                    ...
                    funding_currency = st.selectbox(
                        "Source Currency", options=currencies
                    )
                ...
                if submitted:
                    if amount_usd <= 0:
                        ...
                        if fund_source == "External (Brokerage)":
                            ...
                            st.success(
                                f"Investment order for ${amount_usd:,.2f} in {etf_ticker} has been submitted to your brokerage."
                            )
                            st.info(
                                "Note: In a real app, this would connect to a brokerage API to place the order."
                            )
                        else:
                            # Simulate converting crypto to USD and investing
                            st.success(
                                f"Successfully converted {crypto_amount:.8f} {funding_currency} to ${amount_usd:,.2f} and invested in {etf_ticker}."
                            )
                            st.info(
                                "Note: In a real app, this would execute trades on the exchange and brokerage."
                            )
...
def show_rebalance():
    ...
    if not currencies:
        st.warning(
            "You don't have any assets to rebalance. Add wallets and funds first."
        )
        return
    ...
    with st.form("rebalance_form"):
        ...
        frequency = st.selectbox(
            "Rebalancing Frequency", options=["Manual", "Monthly", "Quarterly"]
        )
        ...
        if submitted:
            if total_allocation != 100:
                ...
                # Show next steps
                st.info(
                    f"Your portfolio will be rebalanced {frequency.lower()}. You can also trigger a manual rebalance below."
                )
...
def show_trade_invest():
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Cryptocurrency Trading", "ETF Investing", "Portfolio Rebalancing"]
    )
