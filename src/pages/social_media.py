...
from src.models.database import SessionLocal, SocialAccount, ScheduledPost
        ...
        for account in accounts:
            with st.container(border=True):
                ...
                with col2:
                    ...
                    st.caption(f"Connected since {account.connected_at.strftime('%Y-%m-%d')}")
    ...
    with st.expander("Connect New Account", expanded=False):
        with st.form("add_social_form"):
            ...
            if platform == "twitter":
                ...
                if submitted:
                    if not username or not api_key or not api_secret or not access_token or not access_secret:
                        ...
                        # Add account
                        if add_social_account(user_id, platform, username, token, token_secret):
...
def show_post_section(user_id):
    ...
    if not accounts:
        st.warning("You need to connect at least one social media account to post. Use the 'Connected Accounts' section above.")
        return
    ...
    with post_tab1:
        ...
        with st.form("manual_post_form"):
            ...
            platform_options = {a.platform: f"{a.platform.capitalize()} (@{a.username})" for a in accounts}
            ...
            media_file = st.file_uploader("Attach Media (optional)", type=["jpg", "jpeg", "png"])
            ...
            # Submit button
            submitted = st.form_submit_button("Post" if not schedule_post else "Schedule")
                ...
                if not selected_platforms:
                    ...
                    if schedule_post:
                        ...
                        for platform in selected_platforms:
                            ...
                            if result['success']:
                                st.success(f"Post scheduled for {platform.capitalize()} at {scheduled_time.strftime('%Y-%m-%d %H:%M')}")
                            else:
                                st.error(f"Error scheduling post for {platform.capitalize()}: {result['message']}")
                    else:
                        # Post immediately
                        for platform in selected_platforms:
                            ...
                            with st.spinner(f"Posting to {platform.capitalize()}..."):
                                try:
                                    ...
                                    if platform == "twitter":
                                        # Get API keys
                                        db = SessionLocal()
                                        api_key, api_secret = get_api_key(db, user_id, "twitter_api")
                                        db.close()
                                        
                                        from src.utils.security import decrypt_data
                                        access_token = decrypt_data(account.encrypted_token)
                                        access_token_secret = decrypt_data(account.encrypted_token_secret)
                                    ...
                                    if result['success']:
                                        st.success(f"Posted to {platform.capitalize()} successfully!")
                                    else:
                                        st.error(f"Error posting to {platform.capitalize()}: {result['message']}")
                                
                                except Exception as e:
                                    st.error(f"Error posting to {platform.capitalize()}: {str(e)}")
    ...
    with post_tab2:
        ...
        with st.form("ai_post_form"):
            ...
            if post_type == "portfolio_update":
                portfolio_value = st.number_input("Portfolio Value (USD)", value=10000.0, step=1000.0)
                daily_change = st.number_input("Daily Change (%)", value=1.5, step=0.1)
                top_assets = st.text_input("Top Performing Assets (comma-separated)", value="BTC, ETH, TSLA")
                ...
                # Market data
                market_data = {
                    "btc_price": st.number_input("Bitcoin Price (USD)", value=69420.0, step=100.0),
                    "eth_price": st.number_input("Ethereum Price (USD)", value=3500.0, step=10.0),
                    "market_trend": st.selectbox("Market Trend", options=["bullish", "bearish", "neutral"])
                }
                sentiment = st.selectbox("Overall Sentiment", options=["positive", "negative", "neutral"])
                ...
                tip_topic = st.selectbox(
                    "Tip Topic",
                    options=["diversification", "dollar_cost_averaging", "risk_management", "emergency_fund", "tax_efficiency"]
                )
                ...
                with st.spinner("Generating post with AI..."):
                    try:
                        ...
                        if post_content.startswith("Error:"):
                            ...
                            st.text_area("Generated Post", value=post_content, height=100)
                            ...
                            with col1:
                                if st.button("Post Now", key="post_ai_now"):
                                    st.info("This would post the content immediately. In the full app, this would call the same posting function as the manual form.")
                            
                            with col2:
                                if st.button("Schedule", key="schedule_ai"):
                                    st.info("This would open a scheduling dialog. In the full app, this would call the same scheduling function as the manual form.")
                            
                            with col3:
                                if st.button("Edit First", key="edit_ai"):
                                    # Set the content in the manual post form
                                    st.session_state['manual_post_content'] = post_content
                                    st.info("In the full app, this would copy the content to the manual post form for editing.")
    ...
    if result['success'] and result['posts']:
        for post in result['posts']:
            with st.container(border=True):
                ...
                with col2:
                    st.markdown(post['content'][:100] + ('...' if len(post['content']) > 100 else ''))
...
def show_sentiment_analysis():
    ...
    query = st.text_input("Enter Topic to Analyze", placeholder="BTC, TSLA, #crypto, etc.")
    ...
    if st.button("Analyze Sentiment"):
        ...
        with st.spinner("Analyzing social media sentiment..."):
            ...
            with col3:
                st.metric("Engagement Level", "High" if sentiment_data['total_posts'] > 200 else "Medium")
            
            # Trending words
            st.subheader("Trending Words")
            st.markdown(" • ".join([f"**#{word}**" for word in sentiment_data['trending_words']]))
