...
class APIConfigManager:
    ...
    def get_api_configs_by_category(category: str) -> List[Dict]:
        ...
        configs = APITester.get_api_config_info()
        return [config for config in configs if config.get("category", "Other") == category]
    ...
    def get_api_credentials(service_id: str) -> Tuple[str, str]:
        ...
        secret = APIConfigManager.get_api_value_from_env(config.get("env_var_secret", ""))
        
        return key, secret
    
    @staticmethod
    def test_api_connection(service_id: str, key: str = None, secret: str = None) -> Tuple[bool, str]:
        """Test API connection"""
        config = APIConfigManager.get_api_config_by_service(service_id)
        if not config:
    ...
    def save_api_credentials(service_id: str, key: str, secret: str = None, db=None, user_id=None, model_preference=None):
        ...
        if config.get("env_var_key") and key:
            env_success = APIConfigManager.update_env_file(config.get("env_var_key"), key)
            if not env_success:
                success = False
                message += f"Failed to update {config.get('env_var_key')} in .env file. "
        
        if config.get("env_var_secret") and secret:
            env_success = APIConfigManager.update_env_file(config.get("env_var_secret"), secret)
            if not env_success:
                success = False
