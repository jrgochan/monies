...
class APITester:
    ...
    def test_binance(api_key: str, api_secret: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                if "restricted location" in str(
                    e
                ) or "geographical restrictions" in str(e):
                    # Don't try regular Binance as it will likely fail for the same reason
                    return (
                        False,
                        f"Failed to connect to Binance.US API due to geographical restrictions: {str(e)}",
                    )
                ...
                try:
                    ...
                    return (
                        False,
                        f"Failed to connect to both Binance.US and Binance APIs: {str(e2)}",
                    )
        except Exception as e:
    ...
    def test_coinbase(api_key: str, api_secret: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            signature = hmac.new(
                secret_bytes, message.encode("utf-8"), digestmod=hashlib.sha256
            ).hexdigest()
            ...
            if response.status_code == 200:
                ...
                return (
                    True,
                    f"Successfully connected to Coinbase API (User: {user_name})",
                )
            else:
                return (
                    False,
                    f"Coinbase API returned status code {response.status_code}: {response.text}",
                )
        except Exception as e:
    ...
    @staticmethod
    def test_alphavantage(api_key: str) -> Tuple[bool, str]:
        """Test connection to Alpha Vantage API"""
        return APITester.test_alpha_vantage(api_key)

    @staticmethod
    def test_alpha_vantage(api_key: str) -> Tuple[bool, str]:
        """Original implementation for Alpha Vantage API testing"""
        try:
            ...
            if "Global Quote" in data and len(data["Global Quote"]) > 0:
                ...
                return (
                    True,
                    "Successfully connected to Alpha Vantage API (note: API call frequency limit reached)",
                )
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "market_data" in data and "current_price" in data["market_data"]:
                    ...
                    return (
                        True,
                        f"Successfully connected to CoinGecko API (BTC price: ${price})",
                    )
                else:
                ...
                return (
                    False,
                    f"CoinGecko API returned status code {response.status_code}: {response.text}",
                )
        except Exception as e:
    ...
    def test_gemini(api_key: str, api_secret: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            payload = {"request": "/v1/account", "nonce": nonce}
    ...
    def test_coinmarketcap(api_key: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return (
                    False,
                    f"CoinMarketCap API returned status code {response.status_code}: {response.text}",
                )
        except Exception as e:
    ...
    def test_hugging_face(api_key: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return (
                    False,
                    f"Hugging Face API returned status code {response.status_code}: {response.text}",
                )
        except Exception as e:
    ...
    def test_fmp(api_key: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return (
                    False,
                    f"Financial Modeling Prep API returned status code {response.status_code}: {response.text}",
                )
        except Exception as e:
    ...
    def test_polygon(api_key: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return (
                    False,
                    f"Polygon.io API returned status code {response.status_code}: {response.text}",
                )
        except Exception as e:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "models" in data:
                    ...
                    return (
                        True,
                        f"Successfully connected to Ollama API (Available models: {', '.join(models[:3])}...)",
                    )
                else:
