...
class DataAggregator:
    ...
    def get_user_data_sources(
        db: Session, user_id: int, category: str = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def update_user_data_source_preferences(
        db: Session, user_id: int, preferences: List[Dict[str, Any]]
    ) -> bool:
        """
        ...
        try:
            ...
            existing_prefs = (
                db.query(UserDataSource).filter(UserDataSource.user_id == user_id).all()
            )
